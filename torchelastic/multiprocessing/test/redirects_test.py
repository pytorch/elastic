...
from torchelastic.multiprocessing.redirects import (
    redirect,
    redirect_stderr,
    redirect_stdout,
)
...
class RedirectsTest(unittest.TestCase):
    ...
    def test_redirect_large_buffer(self):
        def py_print(i):
            print(f"py:{i}")
        ...
        for print_fn in [py_print, c_print]:
            with self.subTest(print_fn=print_fn.__name__):
                with redirect_stdout(stdout_log):
                    for i in range(num):
                        print_fn(i)
