...
3. Number of nodes is allowed to change (elasticity).

**Usage:**

1. Fault tolerant (no elasticity):
...
This launcher starts on agent process which creates and monitors
a local worker group. For multi-node distributed training, simply
start the launcher with the same arguments on all the nodes participating
in training.
...
1. All the important notices section of `torch.distributed.launch`
 apply to this module as well

2. The environment variables necessary to initialize a torch process group
are provided to you by this module. To initialize a process group simply run

::
    >>> torch.distributed.init_process_group(backend="gloo|nccl",
                                             init_method="env://")

3. On failures or membership changes your surviving workers are killed
immediately. Make sure to checkpoint your progress. The frequency of
checkpoints should be proportional to the amount of lost work you can
tolerate.

4. This module only supports homogenous `LOCAL_WORLD_SIZE`. That is,
